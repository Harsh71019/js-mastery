...
const OVERALL_TIER_STYLE: Record<FluencyTier, { label: string; color: string; ring: string }> = {
  Novice:     { label: 'NOVICE',     color: 'text-text-tertiary', ring: 'var(--color-text-tertiary)' },
  Learning:   { label: 'LEARNING',   color: 'text-accent-blue',   ring: 'var(--color-accent-blue)' },
  Proficient: { label: 'PROFICIENT', color: 'text-accent-amber',  ring: 'var(--color-accent-amber)' },
  Advanced:   { label: 'ADVANCED',   color: 'text-accent-green',  ring: 'var(--color-accent-green)' },
  Fluent:     { label: 'FLUENT',     color: 'text-accent-purple', ring: 'var(--color-accent-purple)' },
}
...
const ScoreRing = ({ score, tier }: { score: number; tier: FluencyTier }): React.JSX.Element => {
  ...
  return (
    <ProgressRing progress={score} size={128} strokeWidth={4} color={style.ring} glow>
      <span className={`text-3xl font-bold font-geist tracking-tighter ${style.color}`}>{score}</span>
      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-text-tertiary font-geist opacity-60">{style.label}</span>
    </ProgressRing>
  )
}
            ...
            {(['Fluent', 'Advanced', 'Proficient', 'Learning', 'Novice'] as FluencyTier[]).map((tier) => {
              ...
              return (
                <div key={tier} className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-bold text-text-tertiary uppercase tracking-widest font-geist">{tier}</span>
