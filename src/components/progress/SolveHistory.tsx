...
import { getProblemById, CATEGORIES } from '@/data'
...
export const SolveHistory = ({ solvedProblems }: SolveHistoryProps): React.JSX.Element => {
  ...
  const entries = Object.entries(solvedProblems)
    ...
    .map(([id, entry]): GroupedEntry => ({ problemId: id, solvedAt: entry.solvedAt }))
        ...
        <div key={dateKey}>
          ...
          {dayEntries.map((entry) => {
            ...
            return (
              ...
              >
                <span className="text-text-primary text-sm flex-1 truncate">{problem.title}</span>
